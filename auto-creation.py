...
from python import functions, constants, home_page, team_page, year_page, champion_page
...
# TEAMS = constants.GAME_DATA.loc[constants.GAME_DATA['Playoff Flag'] == 0, 'Team'].unique()
# YEARS = constants.GAME_DATA['Year'].unique()
# YEARS_WEEKS = [(year, constants.GAME_DATA.loc[(constants.GAME_DATA['Year'] == year) & (constants.GAME_DATA['Playoff Flag'] == False), 'Week'].max()) for year in YEARS]
...
def week_pages():
    ...
    for year in years:
        ...
        for week in weeks:
            doc = document()
            doc.add(page_header(active_year=year))
...
# def home_page():
#     doc = document()
#     doc.add(page_header(active_year='home'))
...
# def champion_page():
#     doc = document()
#     doc.add(page_header(active_year='champion'))

#     doc.add(champion_content())

#     with open(f'champion.html','w') as file:
#         file.write(doc.render())
...
#     for team in constants.TEAMS:
#         doc = document()
#         doc.add(page_header(active_year='team'))
